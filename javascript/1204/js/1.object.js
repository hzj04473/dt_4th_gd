...
// (풀이)
// 함수로 전달 될떄, 변수의 값이 복사되어 함수의 매개변수로 전달
// 따라서 함수 내에서 매개변수의 값을 변경하더라도 원본 변수의 값은 변하지 않음
// 왜? 함수내에서 사용되는 변수는 그 함수 스코프 내에서만 유효한 지역변수 이기 떄문
// = x 가 num의 값의 복사본을 가지고 있기 떄문

/**
 * #2. pass by reference (참조에 의한 전달)
 * - 객체나 배열 같은 참조 타입 전딜될 떄 사용
 * - 변수의 메모리 위치(참조)가 전달되므로, 함수 내에서 요소를 변경하면 원본 변수도 변경된다.
 */
...
// JSON.stringfy() 라는 메소드를 사용함
// (가볍게)
// JSON.stringify() 는 "JS 객체 또는 값"을 JSON 형식의 문자열로 변환하는 메소드
// JSON은 데이터를 표현하는 (경량의) 데이터 형식, 키와 값 쌍으로 표현하는데 주로 사용
// - 주로 데이터를 네트워크 요청이나, 파일 저장에 적합한 문자열로 변환 ex) 서버로 보낼떄
...
// obj3고 obj4는 현재 데이터가 같지만, 서로 다른 별도의 객체이다.
